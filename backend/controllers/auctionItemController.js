...
export const addNewAuctionItem = catchAsyncErrors(async (req, res, next) => {
  ...
  if (alreadyOneAuctionActive.length > 20) {
    return next(new ErrorHandler("You already have three active auction.", 400));
  }
  try {
    const cloudinaryResponse = await cloudinary.uploader.upload(
      image.tempFilePath,
      {
        folder: "VIGGIE_BIDOUT_PLATFORM_AUCTIONS",
      }
    );
...
export const getAllItems = catchAsyncErrors(async (req, res, next) => {
  ...
  export const republishItem = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!req.body.startTime || !req.body.endTime) {
      return next(
        new ErrorHandler("Startime and Endtime for republish is mandatory.")
      );
    }
